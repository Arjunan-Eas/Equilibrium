...
/* -----------------------------------------------------------------------------
 * function : Motor_write( speed1, speed2 )
 * INs      : speed1 -> Motor A, speed2 -> Motor B
 * OUTs     : none
 ...
 * date     : 260516
 * -------------------------------------------------------------------------- */
void Motor_write(int16_t speed1, int16_t speed2) {
	// Motor A moving forward
	if(speed1 > 0 && speed1 <= 1000) {
		MOTOR_A_PORT->BSRR |= IN1;	// IN1 HI
		MOTOR_A_PORT->BRR  |= IN2;	// IN2 LO
		TIM5->CCR2 = speed1;	// Sets duty cycle of PWM output
	// Motor A moving backward
	} else if(speed1 < 0 && speed1 >= -1000) {
		MOTOR_A_PORT->BRR  |= IN1;	// IN1 LO
		MOTOR_A_PORT->BSRR |= IN2;	// IN2 HI
		TIM5->CCR2 = (-1*speed1);	// Sets duty cycle of PWM output
	...
	if(speed2 > 0 && speed2 <= 1000) {
		MOTOR_B_PORT->BSRR |= IN3;	// IN3 HI
		MOTOR_B_PORT->BRR  |= IN4;	// IN4 LO
		TIM5->CCR3 = speed2;	// Sets duty cycle of PWM output
	// Motor B moving backward
	} else if(speed2 < 0 && speed2 >= -1000) {
		MOTOR_B_PORT->BRR  |= IN3;	// IN3 LO
		MOTOR_B_PORT->BSRR |= IN4;	// IN4 HI
		TIM5->CCR3 = (-1*speed2);	// Sets duty cycle of PWM output
